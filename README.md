...
**16 names.** Everything else in the package is importable and **unsupported** — reaching for a deeper
...
from arbiter_engine import (
    ...
    Case,                       # one problem, from the finding that opened it to its close
...
**A case is the problem an operator works; a `Problem` is still one finding.** `open_case` opens
one on a subject and a declared indicator, every later `check` records itself into it, and the
caller attaches what each other stage did -- the ranking, the plan, the execution, the adoption --
with `attach_stage`, by reference and with any decline. It resolves after the model's
`cases.consecutive_checks` checks in a row that looked at the indicator and found nothing at or
above `cases.severity`; a check that could not look restarts the count. The book calls no stage
itself, lives beside the ledger, and `case_book` prints how many cases opened and how many resolved.
...
**v0.2.** 99 Python files, 97 modules importing on the declared dependencies alone, 16 supported
...
**And the count is of SUBMODULES: the root package is not one of them.** Walking `arbiter_engine` for what it contains gives 97; adding the package you imported to reach them gives 98. Both are honest and they are answers to different questions, so a reader who recounts and gets one more has not found a defect — they have used the other convention. Stated because someone did exactly that from outside, and a number published without its predicate can only be agreed with or disagreed with, never checked. **Every figure in this paragraph moved by TWO when the reference producer landed, by one when `assumptions` did, and by one again when the case book did, and the `assumptions` move went unnoticed for two rounds** -- the guard that compares them to the artifact only runs when a built tree exists, so it is silent in every session that does not build one. A check conditioned on an artifact is not a check that runs.
...
`margin_book.yaml` is the only one declaring work the engine has not been handed yet -- an
outside forecaster is expected to supply a prediction, and is then judged by the same eight axioms --
and `pump_tank_dynamics.yaml` is the only one that declares DYNAMICS: a `transition:` with a
